...
import { Loader2, Image as ImageIcon, RefreshCw, Wand2, ArrowRight, Volume2, Download, CheckSquare, Square, Maximize2, RotateCcw, RotateCw, History, PlayCircle, Type, Film, Sparkles, Tag, Plus, X, Edit3 } from 'lucide-react';
      ...
      <div className="flex flex-col gap-8 pb-8 border-b border-white/5">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
           ...
           <div className="flex items-center gap-1 bg-black/40 p-1 rounded-xl border border-white/10 shadow-inner">
              <button onClick={undo} disabled={!canUndo} className="p-2.5 rounded-lg hover:bg-white/10 text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="撤销"><RotateCcw className="w-4 h-4" /></button>
              <button onClick={redo} disabled={!canRedo} className="p-2.5 rounded-lg hover:bg-white/10 text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="重做"><RotateCw className="w-4 h-4" /></button>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-white/[0.02] border border-white/5 rounded-2xl backdrop-blur-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleExportClick} disabled={story.scenes.length === 0} className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg transition-all shadow-lg shadow-indigo-500/20 active:scale-95"><Download className="w-4 h-4" /> Export</button>
          </div>
        </div>
      </div>
...
// ... Helper Components (ScrollReveal, SubtitleOverlay, SceneCard) remain similar, 
// just updating SceneCard to use local state logic properly
...
const SceneCard: React.FC<{ 
  ...
  // Local state for editing to prevent constant re-renders from global store during typing
  ...
  // Sync local state when prop changes (from undo/redo)
  useEffect(() => {
  ...
  const handlePolish = async (type: 'narrative' | 'visual') => {
    // Note: polishText is a service function, not a hook, so safe to use here
    if (type === 'narrative') {
  ...
  if (isComicMode) {
    // ... Comic Mode Render (Identical logic, shortened for brevity in this response)
     return (
        <div className={`group/card h-full flex flex-col relative bg-transparent transition-all duration-300 ${isSelected ? 'ring-4 ring-indigo-500/50' : ''}`}>
           <div className={`flex-1 border-4 bg-white shadow-[8px_8px_0px_rgba(0,0,0,1)] relative overflow-hidden flex flex-col transition-colors duration-300 ${isSelected ? 'border-indigo-600' : 'border-black'}`}>
              <div className="relative flex-1 bg-slate-100 min-h-[200px] overflow-hidden">
                 {scene.imageUrl ? (
                    <>
                       <img src={scene.imageUrl} alt={`Panel ${index + 1}`} className={`w-full h-full object-cover transition-transform duration-700 ease-out ${isSelected ? 'scale-110' : 'hover:scale-105'}`} />
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 p-4 text-center">
                       {scene.isLoadingImage ? (
                          <div className="flex flex-col items-center gap-2"><Loader2 className="w-8 h-8 animate-spin text-black" /><span className="text-xs font-bold text-black uppercase">Drawing...</span></div>
                       ) : (
                          <div className="flex flex-col items-center gap-2"><span className="text-xs font-bold">Image Failed</span><button onClick={onRetry} className="text-[10px] underline hover:text-black">Retry</button></div>
                       )}
                    </div>
                 )}
        ...
        <div className="flex flex-col justify-between border-t border-white/5 bg-gradient-to-b from-[#13161f] to-[#0f111a] flex-1 p-6 xl:p-8 xl:border-t-0 xl:border-l">
           <div className="space-y-4">
              <div className="flex items-center justify-between">
                 <div className="flex items-center gap-4"><span className="text-xl font-black text-white/10 select-none">{String(index + 1).padStart(2, '0')}</span><span className="px-2 py-0.5 rounded border border-white/5 bg-white/5 text-[10px] font-bold text-indigo-300 uppercase tracking-widest">SCENE</span></div>
                 {!isEditingText && <button onClick={() => setIsEditingText(true)} className="p-2 hover:bg-white/5 rounded-lg text-slate-500 hover:text-white transition-colors" title="Edit Script"><Edit3 className="w-4 h-4" /></button>}
              </div>
              {isEditingText ? (
                  <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2">
                     <div className="space-y-2"><label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex justify-between">Narrative <button onClick={() => handlePolish('narrative')} disabled={isPolishingNarrative} className="text-indigo-400 hover:text-indigo-300 flex items-center gap-1">{isPolishingNarrative ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />} AI Polish</button></label><textarea value={editNarrative} onChange={(e) => setEditNarrative(e.target.value)} className="w-full h-24 bg-black/20 border border-white/10 rounded-xl p-3 text-sm text-white focus:ring-1 focus:ring-indigo-500 outline-none resize-none" /></div>
                     <div className="space-y-2"><label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex justify-between">Visual Prompt <button onClick={() => handlePolish('visual')} disabled={isPolishingVisual} className="text-indigo-400 hover:text-indigo-300 flex items-center gap-1">{isPolishingVisual ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />} AI Polish</button></label><textarea value={editVisual} onChange={(e) => setEditVisual(e.target.value)} className="w-full h-24 bg-black/20 border border-white/10 rounded-xl p-3 text-xs text-slate-300 focus:ring-1 focus:ring-indigo-500 outline-none resize-none font-mono" /></div>
                 ...
                 <div className="space-y-3">
                    ...
                    <div className="bg-black/20 rounded-xl p-4 border border-white/5"><h5 className="text-[9px] font-bold text-slate-600 uppercase tracking-widest mb-2 flex items-center gap-2"><Maximize2 className="w-3 h-3" /> Visual Prompt</h5><p className="text-xs text-slate-500 font-mono italic leading-relaxed line-clamp-3 hover:line-clamp-none transition-all cursor-text select-all">{scene.visual_prompt}</p></div>
                 </div>
              )}
           ...
           <div className="mt-4 border-t border-white/5 flex flex-wrap gap-3 pt-6">
              <div className="flex items-center gap-2">{scene.audioUrl ? (<div className="flex items-center gap-2 bg-slate-800/80 rounded-full pl-3 pr-1 py-1.5 border border-white/5 shadow-sm"><Volume2 className="w-3.5 h-3.5 text-indigo-400" /><audio src={scene.audioUrl} controls className="h-6 w-24 opacity-80 hover:opacity-100 transition-opacity" /></div>) : (<button onClick={onGenerateAudio} disabled={scene.isLoadingAudio} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-xs font-medium text-slate-400 hover:text-white transition-colors border border-white/5">{scene.isLoadingAudio ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Volume2 className="w-3.5 h-3.5" />} Audio</button>)}</div>
              {scene.videoUrl ? (<button className="flex items-center gap-2 px-3 py-2 rounded-lg bg-indigo-500/10 text-indigo-300 text-xs font-bold border border-indigo-500/20 cursor-default"><Film className="w-3.5 h-3.5" /> Video Ready</button>) : (<button onClick={onGenerateVideo} disabled={scene.isLoadingVideo || !scene.imageUrl} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-xs font-medium text-slate-400 hover:text-purple-300 transition-colors border border-white/5 disabled:opacity-50">{scene.isLoadingVideo ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Film className="w-3.5 h-3.5" />} Generate Video (Veo)</button>)}
           </div>
        </div>
      </div>
    </div>
